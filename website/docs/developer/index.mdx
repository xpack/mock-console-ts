...
# How to build the xPack Mock Console
...
```sh
npm --prefix ~/Work/npm-modules/mock-console-ts.git install
```
...
```sh
npm --prefix ~/Work/npm-modules/mock-console-ts.git link
```
...
```sh
npm --prefix ~/Work/npm-modules/mock-console-ts.git run compile-watch
```
...
> @xpack/mock-console@{customField('version')} fix
...
```sh
npm --prefix ~/Work/npm-modules/mock-console-ts.git install
npm --prefix ~/Work/npm-modules/mock-console-ts.git run test
```
...
[GitHub Actions](https://github.com/xpack/mock-console-ts/actions) on Ubuntu,
